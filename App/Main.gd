...
func _ready() -> void:
	...
	Global.CALNotify.connect(MessageReceive)
...
func load_all_data() -> void:
	...
	if config.has_section("Data"):
		var Data = config.get_section_keys("Data")
		for VName in Data:
			var VRate = config.get_value("Data", VName)
			
			# Instantiate the scene
			var Ins = CAL.instantiate()
			
			# Add the instance as a child to the container node
			%HBC.add_child(Ins)
		...
		print("No 'Data' section found in config file")
...
func NoOProductDataBase():

	
	Global.TotalData # = intbase + floatbase
	print(Global.TotalData)
	
	#print(" TotalNoOProductDataBase ",intbase,floatbase)
...
func MessageReceive(message: String) -> void:
	$Notify2.text = message
	$Out.stop()
	$Out.start()
